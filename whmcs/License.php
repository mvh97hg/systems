...
class License
{
    ...
    protected function makeCall($url, $query_string, $timeout = 5)
    {
		...
		*/
        ...
        $results["registeredname"] = "V4U.VN";
        $results["productname"] = "WHMCS V4U.VN";
        $results["productid"] = "5";
    ...
    public function isDevLicense()
    {
        $licenseKeyParts = explode("-", $this->getLicenseKey(), 2);
        $prefix = isset($licenseKeyParts[0]) ? $licenseKeyParts[0] : "";
        return in_array($prefix, array("Dev", "Beta", "Security", "Trial"));
    }
    ...
    public function getRegistrationDate($showday = false)
    {
        $registrationDate = $this->getKeyData("regdate");
        if (!$registrationDate) {
            return "";
        }
